...
class LibManager():
  ...
  def _close_native_lib(self, lib, ctx, tr):
    ...
    close_addr = ctx.mem.read_mem(2, lib_base - 10) # LVO_Close
  ...
  def _rtinit_native_lib(self, lib, ctx, tr, init_ptr):
    ...
    # DEBUG
    addr = seg_list << 2
    sl_len = ctx.mem.read_mem(2,addr-4)
    sl_next = ctx.mem.read_mem(2,addr)
  ...
  def _free_native_lib(self, lib, ctx, tr):
    ...
    exec_base = ctx.mem.read_mem(2,4)
    lib_base = lib.addr_base
    expunge_addr = ctx.mem.read_mem(2, lib_base - 16) # LVO_Expunge
    if expunge_addr != 0:
